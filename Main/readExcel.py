...
class Action(object):
    ...
    def __init__(self):
        self.data = GetData()
        self.casenames = self.data.getCaseNames()
    
    def run(self):
        '''从GetData()类中获取所有的测试用例名称
        '''
        for casename in self.casenames:
            self.method(self.data.getCaseStep(casename).get(casename))
    
    def method(self,arg):
        '''调用keywordswitch中的runKey()方法用于执行用例的执行
        @param arg: type:list
        ...
        for i in arg:
            ks = keywordswitch(driver)
            ks.runKey(i[0],i[1],i[2],i[3])
